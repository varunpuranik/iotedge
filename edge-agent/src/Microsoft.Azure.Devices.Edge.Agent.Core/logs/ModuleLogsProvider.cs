...
//    using Newtonsoft.Json;
    ...
    //public class ModuleLogsProvider : ILogsProcessor
    //{
    //    public async Task<Stream> GetLogsAsStream(LogsRequest logsRequest, CancellationToken cancellationToken) => throw new NotImplementedException();

    //    public async Task<IEnumerable<ModuleLogMessage>> GetLogs(LogsRequest logsRequest, CancellationToken cancellationToken) => throw new NotImplementedException();
    //}
...
//        {
//            string regexPattern = @"^(<(?<logLevel>\d)>)?\s*((?<timestamp>\d{4}-\d{2}-\d{2}\s\d{2}:\d{2}:\d{2}.\d{3}\s[+-]\d{2}:\d{2})\s)?";
...
//                    if(int.TryParse(ll, out int parsedLogLevel))
//                    {
